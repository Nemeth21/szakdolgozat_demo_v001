...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (storedEmail) {
      ...
      router.replace("/auth"); // 🔹 Ha nincs email, irányítás a login oldalra
    }
  }, []);

  const handleLogout = () => {
    localStorage.removeItem("userEmail"); // 🔹 Email törlése
    localStorage.setItem("lastPage", "/dashboard"); // 🔹 Az utolsó oldal mentése logout előtt
    router.replace("/auth"); // 🔹 Logout után átirányítás loginra
  };
